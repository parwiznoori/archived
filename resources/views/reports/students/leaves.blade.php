<table>
    <thead>
    <tr>
        <th colspan="23">{{trans('general.leaves_list')}} </th>
    ...
    <tr>
        ...
        <th>{{trans('general.approved_leave')}}</th>
        <th>{{trans('general.note')}}</th>
        <th>{{trans('general.end_leave')}}</th>
    </tr>
    </thead>
    <tbody>
    @foreach($students as $student)
        <tr>
            ...
            <td>{{ $student->note }}</td>
            <td>{{ $student->end_leave == 1 ?  trans('general.yes') : trans('general.no') }}</td>
        </tr>
    @endforeach
    </tbody>
</table>